-- ============================================================
-- MÓDULO AUDITORIA — Gestão Eklesia
-- Cria/recria a tabela audit_logs com o schema correto.
-- ATENÇÃO: Se a tabela já foi criada pelo modulo_configuracoes.sql
-- com o nome created_at errado, este script a recria corretamente.
-- ============================================================

-- 1. Remover tabela antiga (criada com schema incorreto)
DROP TABLE IF EXISTS public.audit_logs CASCADE;

-- 2. Criar com schema exato esperado pelo código
CREATE TABLE public.audit_logs (
  id               UUID    PRIMARY KEY DEFAULT gen_random_uuid(),

  -- Campos do schema atual (route.ts POST + page.tsx)
  usuario_id       UUID    REFERENCES auth.users(id) ON DELETE SET NULL,
  usuario_email    VARCHAR(255),
  acao             VARCHAR(100) NOT NULL,
  modulo           VARCHAR(100) NOT NULL,
  area             VARCHAR(100),
  tabela_afetada   VARCHAR(100),
  registro_id      UUID,
  descricao        TEXT,
  dados_anteriores JSONB,
  dados_novos      JSONB,
  ip_address       INET,
  user_agent       TEXT,
  status           VARCHAR(20) DEFAULT 'sucesso',
  mensagem_erro    TEXT,

  -- Campos do schema legado (fallback ministry-based na route.ts)
  ministry_id      UUID    REFERENCES public.ministries(id) ON DELETE CASCADE,

  -- Campos do schema legado empresa-based (fallback antigo)
  action           VARCHAR(50),
  resource_type    VARCHAR(100),
  resource_id      UUID,
  old_data         JSONB,
  new_data         JSONB,
  changes          JSONB,
  status_code      INTEGER,
  error_message    TEXT,

  -- Coluna de data com nome exato que o código usa
  data_criacao     TIMESTAMP WITH TIME ZONE DEFAULT NOW(),

  CONSTRAINT valid_acao CHECK (acao IN (
    'criar', 'editar', 'deletar', 'visualizar', 'exportar', 'importar',
    'responder', 'atualizar_status', 'atualizar_permissoes',
    'login', 'logout', 'download', 'upload', 'outro'
  )),
  CONSTRAINT valid_status CHECK (status IN ('sucesso', 'erro', 'aviso'))
);

-- 3. Índices para performance (usados pelas queries do código)
CREATE INDEX IF NOT EXISTS idx_audit_usuario        ON public.audit_logs(usuario_id);
CREATE INDEX IF NOT EXISTS idx_audit_modulo         ON public.audit_logs(modulo);
CREATE INDEX IF NOT EXISTS idx_audit_acao           ON public.audit_logs(acao);
CREATE INDEX IF NOT EXISTS idx_audit_data           ON public.audit_logs(data_criacao DESC);
CREATE INDEX IF NOT EXISTS idx_audit_usuario_data   ON public.audit_logs(usuario_id, data_criacao DESC);
CREATE INDEX IF NOT EXISTS idx_audit_tabela         ON public.audit_logs(tabela_afetada, registro_id);
CREATE INDEX IF NOT EXISTS idx_audit_ministry_id    ON public.audit_logs(ministry_id);

-- 4. RLS
ALTER TABLE public.audit_logs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "users_view_own_audit_logs"  ON public.audit_logs;
DROP POLICY IF EXISTS "users_create_audit_logs"    ON public.audit_logs;
DROP POLICY IF EXISTS "audit_logs_ministry_select" ON public.audit_logs;
DROP POLICY IF EXISTS "audit_logs_service_insert"  ON public.audit_logs;

-- Cada usuário vê seus próprios logs
CREATE POLICY "users_view_own_audit_logs"
  ON public.audit_logs FOR SELECT
  USING (auth.uid() = usuario_id);

-- Admin/manager do ministério também vê os logs do ministério
CREATE POLICY "audit_logs_ministry_select"
  ON public.audit_logs FOR SELECT
  TO authenticated
  USING (
    ministry_id IN (
      SELECT ministry_id FROM public.ministry_users
        WHERE user_id = auth.uid() AND role IN ('admin', 'manager')
    )
  );

-- Qualquer usuário autenticado pode inserir (INSERT feito pelo cliente e pela API)
CREATE POLICY "users_create_audit_logs"
  ON public.audit_logs FOR INSERT
  WITH CHECK (auth.uid() = usuario_id);

-- 5. Permissões explícitas
GRANT SELECT, INSERT ON public.audit_logs TO authenticated;

-- ============================================================
-- FIM DO SCRIPT
-- ============================================================
