'use client';

import { useState } from 'react';
import PageLayout from '@/components/PageLayout';
import Tabs from '@/components/Tabs';
import Section from '@/components/Section';
import { useRequireModulo } from '@/hooks/useRequireModulo';

export default function PreCadastroPage() {
  const { ctx, bloqueado } = useRequireModulo('secretaria');
  const [activeTab, setActiveTab] = useState('pendentes');

  if (ctx.loading) return <div className="p-8">Carregando...</div>;
  if (bloqueado) return null;

  const tabs = [
    { id: 'pendentes', label: 'Pendentes', icon: '⏳' },
    { id: 'confirmados', label: 'Confirmados', icon: '✅' },
    { id: 'rejeitados', label: 'Rejeitados', icon: '❌' }
  ];

  return (
    <PageLayout
      title="Pré-cadastro"
      description="Gerenciar pré-cadastros aguardando confirmação"
      activeMenu="pre-cadastro"
    >
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-lg shadow border-l-4 border-yellow-500">
          <p className="text-gray-600 text-sm">Pré-cadastros Pendentes</p>
          <p className="text-3xl font-bold text-[#123b63] mt-2">0</p>
        </div>

        <div className="bg-white p-6 rounded-lg shadow border-l-4 border-green-500">
          <p className="text-gray-600 text-sm">Pré-cadastros Confirmados</p>
          <p className="text-3xl font-bold text-green-600 mt-2">0</p>
        </div>

        <div className="bg-white p-6 rounded-lg shadow border-l-4 border-red-500">
          <p className="text-gray-600 text-sm">Pré-cadastros Rejeitados</p>
          <p className="text-3xl font-bold text-red-600 mt-2">0</p>
        </div>
      </div>

      <Tabs tabs={tabs} activeTab={activeTab} onTabChange={setActiveTab}>
        <Section icon="📝" title="Solicitações Pendentes">
          <p className="text-gray-500 text-center py-8">Nenhum pré-cadastro pendente</p>
          <button className="mt-4 bg-[#123b63] text-white px-6 py-2 rounded hover:bg-[#0f2a45] transition w-full">
            + Novo Pré-cadastro
          </button>
        </Section>
      </Tabs>
    </PageLayout>
  );
}
