...
export async function POST(request: NextRequest) {
  try {
    ...
    const registroAceite = await aceitesService.registrarAceite({
      ...
      hash_documento: doc.hash_sha256 || 'HASH_INICIAL_TRIAL',
      ip_address: ipAddress,
      user_agent: userAgent,
      payload_aceite: { origem: 'TRIAL_ONBOARDING' },
    });
    ...
    return NextResponse.json(
      { success: false, error: error?.message || 'Erro ao registrar aceite de documento no Trial.' },
      { status: 400 }
    );
  }
}
