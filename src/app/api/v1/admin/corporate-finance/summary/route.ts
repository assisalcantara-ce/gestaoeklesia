...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = await CorporateFinanceService.getFinancialSummary(supabase, filter)
    const evolution = await CorporateFinanceService.getMonthlyEvolution(supabase, 6)
    ...
    console.error('Erro na API Corporate Finance Summary:', err)
