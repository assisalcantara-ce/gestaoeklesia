    ...
    <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col items-center justify-center p-4 md:p-6">
      ...
      <div className="relative w-full max-w-3xl bg-gray-900/90 border border-gray-800 rounded-2xl shadow-2xl overflow-hidden backdrop-blur-md flex flex-col">
        ...
        <div className="p-6 border-t border-gray-800 bg-gray-950/80 space-y-5">
          ...
          {/* Se for documento institucional e o usuário não for ADMINISTRADOR */}
          {documentoAtual && (documentoAtual.escopo === 'INSTITUCIONAL' || ['CONTRATO_SERVICO', 'ADITIVO'].includes(documentoAtual.tipo)) && !userCtx.isAdmin ? (
            <div className="space-y-4 pt-2">
              <div className="p-4 bg-amber-950/40 border border-amber-800/60 rounded-xl flex items-start gap-3 text-amber-200">
                <AlertCircle size={20} className="shrink-0 text-amber-400 mt-0.5" />
                <p className="text-xs leading-relaxed">
                  Este contrato institucional aguarda assinatura de um usuário com perfil <strong>ADMINISTRADOR</strong> da instituição.
                </p>
              </div>
              <div className="flex items-center justify-between pt-2">
                ...
                <a
                  href="/dashboard"
                  className="flex items-center gap-2 px-6 py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-200 rounded-xl text-xs font-semibold border border-gray-700 transition"
                >
            ...
            /* Botão Aceitar e Continuar para usuários autorizados ou termos individuais */
            <div className="flex items-center justify-between pt-2">
              <span className="text-[11px] text-gray-500">
                {documentoAtual ? `Hash SHA-256: ${documentoAtual.hash_sha256?.substring(0, 16) || 'Oficial'}...` : ''}
              </span>
